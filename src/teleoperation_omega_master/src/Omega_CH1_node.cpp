...
public:
	...
	int init()
	{
	    ...
	    master_dt_pub_ = node_->advertise<std_msgs::Float64>("Master/CH1/Dt",10);
	    master_position_cmd_pub_ = node_->advertise<geometry_msgs::TwistStamped>("/Master/CH1/command/position", 10);
	    master_velocity_cmd_pub_ = node_->advertise<geometry_msgs::PoseStamped>("/Master/CH1/command/velocity",10);
	    indexing_button_pub_ = node_->advertise<std_msgs::Bool>("/Master/CH1/indexing", 10);
	    force_feedback_sub_ = node_->subscribe<geometry_msgs::WrenchStamped>("/Master/CH1/FeedbackForce", 10, &Omega_CH1::ForceFeedbackCallback, this);
...
{
  ros::init(argc, argv, "Omega_CH1_node");

  Omega_CH1 omega_ch1;
  if (omega_ch1.init())
	{
    ROS_FATAL("Omega_CH1_node initialization failed");
		return -1;
	}
	...
	omega_ch1.ForceFeedback_[0] = 0.0;
	omega_ch1.ForceFeedback_[1] = 0.0;
	omega_ch1.ForceFeedback_[2] = 0.0;
  ...
  if (dhdOpenSerial(SERIAL_NUMBER_SIGMA7) < 0) {
    ...
    dhdGetGripperAngleDeg(&GripperAngle);
    ...
    MasterPositionCmd.header.frame_id = "/Omega_CH1";
	MasterPositionCmd.header.stamp = ros::Time::now();
	static geometry_msgs::TwistStamped prevMasterPositionCmd;


    geometry_msgs::PoseStamped MasterVelocityCmd;
    
    MasterVelocityCmd.header.frame_id = "/Omgea_CH1";
    ...
    if(Getbutton){
		omega_ch1.indexing_ = true;

			// Force Feedback reflect to the omega6
		final_force[0] += omega_ch1.ForceFeedback_[0];
		final_force[1] += omega_ch1.ForceFeedback_[1];
		final_force[2] += omega_ch1.ForceFeedback_[2];
		...
		*/
		...
		// Low Pass Filtering
		double alpha = 0.3;
		MasterVelocityCmd.pose.position.x = omega_ch1.LowPassFilter(position_based_velocity_x_,prevMasterVelocityCmd.pose.position.x,alpha);
		MasterVelocityCmd.pose.position.y = omega_ch1.LowPassFilter(position_based_velocity_y_,prevMasterVelocityCmd.pose.position.y,alpha);
		MasterVelocityCmd.pose.position.z = omega_ch1.LowPassFilter(position_based_velocity_z_,prevMasterVelocityCmd.pose.position.z,alpha);

		prevMasterVelocityCmd = MasterVelocityCmd;

	}
	else{
		omega_ch1.indexing_ = false;
    ...
    omega_ch1.master_position_cmd_pub_.publish(MasterPositionCmd);
    omega_ch1.master_velocity_cmd_pub_.publish(MasterVelocityCmd);
    omega_ch1.master_dt_pub_.publish(master_dt);
    indexing.data = omega_ch1.indexing_;
    omega_ch1.indexing_button_pub_.publish(indexing);
